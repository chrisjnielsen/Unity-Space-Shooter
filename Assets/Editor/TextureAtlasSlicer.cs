...
{     
    ...
    private void UseSelectedTexture()
    {
        ...
        if (selectedTexture != null)
        {
            ...
            if (importer)
            {
                ...
                {
                    // Try to get tet formatted sprite layout
    ...
    private void SetWantedDimensions()
    {
        wantedWidth = 0;
        wantedHeight = 0;
    ...
    private SubTexture GetSubTexture(XmlNode childNode)
    {
        ...
        string _name = "ERROR";
        if (childNode.Attributes["name"] != null) _name = childNode.Attributes["name"].Value;
        else Debug.LogError("'name' attribute not found on childNode");

        if (childNode.Attributes["width"] != null) w = Convert.ToInt32(childNode.Attributes["width"].Value);
        else Debug.LogError("'width' attribute not found on childNode");

        if (childNode.Attributes["height"] != null) h = Convert.ToInt32(childNode.Attributes["height"].Value);
        else Debug.LogError("'height' attribute not found on childNode");

        if (childNode.Attributes["x"] != null) _x = Convert.ToInt32(childNode.Attributes["x"].Value);
        else Debug.LogError("'x' attribute not found on childNode");

        if (childNode.Attributes["y"] != null) _y = Convert.ToInt32(childNode.Attributes["y"].Value);
        else Debug.LogError("'y' attribute not found on childNode");
    ...
    public void OnGUI()
    {
        ...
        EditorGUI.BeginDisabledGroup(focusedWindow != this);
        {
            ...
            EditorGUI.BeginDisabledGroup((importer.textureType != TextureImporterType.Sprite && importer.textureType != TextureImporterType.Default));
            {
                ...
                EditorGUI.BeginDisabledGroup(xmlAsset == null || needsToResizeTexture || subTextures == null || subTextures.Length == 0);
